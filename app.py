...
with st.sidebar:
    ...
    st.divider()
    st.caption("v5.0 - Verifica Integrata")
...
# --- 2. GESTIONE & VERIFICA (SOLUZIONE SMART) ---
    ...
    with tab1:
        st.subheader("Registra o Aggiorna un Veicolo")
        with st.form("form_mezzo", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Salva e Storicizza"):
                if targa and ass:
                    ...
                    st.error("Targa e Associazione sono obbligatori.")

    with tab2:
        st.subheader("🔍 Strumenti di verifica esterna gratuita")
        st.info("Inserisci una targa per aprire i link ufficiali di controllo.")
        targa_v = st.text_input("Inserisci targa da controllare:").upper().strip()
        
        if targa_v:
            st.write(f"Verifica per la targa: **{targa_v}**")
            v1, v2 = st.columns(2)
            
            # LINK 1: PORTALE DELL'AUTOMOBILISTA (Revisione)
            url_rev = f"https://www.ilportaledellautomobilista.it/web/portale-automobilista/verifica-ultima-revisione"
            v1.link_button("🛠️ Verifica Ultima Revisione", url_rev, help="Ti porta sul sito del Ministero. Dovrai reinserire la targa.")
            
            # LINK 2: PORTALE DELL'AUTOMOBILISTA (Assicurazione)
            url_ass = f"https://www.ilportaledellautomobilista.it/web/portale-automobilista/verifica-copertura-rc"
            v2.link_button("🛡️ Verifica Copertura RCA", url_ass)
            
            st.divider()
            st.caption("Nota: Questi portali non permettono l'invio diretto della targa via URL per motivi di sicurezza (CAPTCHA), ma sono la fonte più affidabile e gratuita disponibile.")
...
# --- 3. STORICO & EXCEL ---
    ...
    st.divider()
    st.subheader("Ricerca Storico per Targa")
